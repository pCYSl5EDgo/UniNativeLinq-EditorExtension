...
{
    public sealed class OrderByIComparer : ITypeDictionaryHolder, IApiExtensionMethodGenerator
    {
        public OrderByIComparer(ISingleApi api)
        {
            Api = api;
        }
        ...
        private void GenerateEach(string name, bool isSpecial, TypeDefinition @static, ModuleDefinition mainModule, ModuleDefinition systemModule)
        {
            ...
            var Func = new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System.Collections.Generic", "IComparer`1")))
            {
                GenericArguments = { T }
            };

            var TComparer = new GenericInstanceType(mainModule.GetType("UniNativeLinq", "OrderByFromIComparer`1"))
            {
                GenericArguments = { T }
            };
