...
{
    public sealed class SumUInt64 : IApiExtensionMethodGenerator, ITypeDictionaryHolder
    {
        public SumUInt64(ISingleApi api)
        {
            Api = api;
        }
        ...
        private static void GenerateGeneric(TypeDefinition @static, ModuleDefinition mainModule) => mainModule.TypeSystem.UInt64.GenerateSum_Generic(@static);

        private void GenerateEach(string name, bool isSpecial, TypeDefinition @static, ModuleDefinition mainModule)
        {
            var returnType = mainModule.TypeSystem.UInt64;
            var method = new MethodDefinition(Api.Name, Helper.StaticMethodAttributes, returnType)
            {
