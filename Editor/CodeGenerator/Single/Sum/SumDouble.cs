...
{
    public sealed class SumDouble : IApiExtensionMethodGenerator, ITypeDictionaryHolder
    {
        public SumDouble(ISingleApi api)
        {
            Api = api;
        }
        ...
        private static void GenerateGeneric(TypeDefinition @static, ModuleDefinition mainModule) => mainModule.TypeSystem.Double.GenerateSum_Generic(@static);

        private void GenerateEach(string name, bool isSpecial, TypeDefinition @static, ModuleDefinition mainModule)
        {
            var returnType = mainModule.TypeSystem.Double;
