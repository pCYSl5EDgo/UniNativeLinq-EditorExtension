...
{
    public sealed class SumSingle : IApiExtensionMethodGenerator, ITypeDictionaryHolder
    {
        public SumSingle(ISingleApi api)
        {
            Api = api;
        }
        ...
        private static void GenerateGeneric(TypeDefinition @static, ModuleDefinition mainModule) => mainModule.TypeSystem.Single.GenerateSum_Generic(@static);

        private void GenerateEach(string name, bool isSpecial, TypeDefinition @static, ModuleDefinition mainModule)
        {
            var returnType = mainModule.TypeSystem.Single;
