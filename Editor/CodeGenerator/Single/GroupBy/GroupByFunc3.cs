...
{
    public sealed class GroupByFunc3 : ITypeDictionaryHolder, IApiExtensionMethodGenerator
    {
        public GroupByFunc3(ISingleApi api)
        {
            Api = api;
        }
        ...
        private void GenerateEach(string name, bool isSpecial, TypeDefinition @static, ModuleDefinition mainModule, ModuleDefinition systemModule)
        {
            ...
            var KeyFunc = new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`2")))
            {
                GenericArguments = { T, TKey }
            };

            var TKeyFunc = new GenericInstanceType(mainModule.GetType("UniNativeLinq", "DelegateFuncToStructOperatorAction`2"))
            {
                GenericArguments = { T, TKey }
            };
            ...
            var ElementFunc = new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`2")))
            {
                GenericArguments = { T, TElement }
            };

            var TElementFunc = new GenericInstanceType(mainModule.GetType("UniNativeLinq", "DelegateFuncToStructOperatorAction`2"))
            {
                GenericArguments = { T, TElement }
            };

            var Comparer = new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`3")))
            {
