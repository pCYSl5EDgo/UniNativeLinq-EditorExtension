...
{
    public sealed class UnionOperator : IApiExtensionMethodGenerator, ITypeDictionaryHolder
    {
        ...
        public void Generate(IEnumerableCollectionProcessor processor, ModuleDefinition mainModule, ModuleDefinition systemModule, ModuleDefinition unityModule)
        {
            ...
            mainModule.Types.Add(@static = mainModule.DefineStatic(nameof(UnionOperator) + "Helper"));
            var count = Api.Count;
        ...
        private void GenerateSpecialSpecial(string rowName, string columnName, ModuleDefinition mainModule, ModuleDefinition systemModule, MethodDefinition method)
        {
            ...
            var TEqualityComparer = DefineEqualityComparer(mainModule, method, T);
        ...
        private static GenericParameter DefineEqualityComparer(ModuleDefinition mainModule, MethodDefinition method, GenericParameter T)
        {
            var TEqualityComparer = new GenericParameter("TEqualityComparer", method)
            {
                ...
                Constraints = {
                    new GenericInstanceType(mainModule.GetType("UniNativeLinq", "IRefFunc`3"))
                    {
                        GenericArguments =
                        {
                            T, T, mainModule.TypeSystem.Boolean,
                        }
                    }
                }
            };
            method.GenericParameters.Add(TEqualityComparer);
            return TEqualityComparer;
        }
