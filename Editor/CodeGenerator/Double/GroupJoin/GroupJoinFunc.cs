...
{
    public sealed class GroupJoinFunc : IApiExtensionMethodGenerator, ITypeDictionaryHolder
    {
        public readonly IDoubleApi Api;
        public GroupJoinFunc(IDoubleApi api) => Api = api;
        public Dictionary<string, TypeDefinition> Dictionary { private get; set; }

        public void Generate(IEnumerableCollectionProcessor processor, ModuleDefinition mainModule, ModuleDefinition systemModule, ModuleDefinition unityModule)
        {
            if (!processor.TryGetEnabled("GroupJoin", out var enabled) || !enabled) return;
            var array = processor.EnabledNameCollection.Intersect(Api.NameCollection).ToArray();
            if (!Api.ShouldDefine(array)) return;
            TypeDefinition @static;
            mainModule.Types.Add(@static = mainModule.DefineStatic(nameof(GroupJoinFunc) + "Helper"));
            var count = Api.Count;
        ...
        private void GenerateEachPair(string rowName, bool isRowSpecial, string columnName, bool isColumnSpecial, TypeDefinition @static, ModuleDefinition mainModule, ModuleDefinition systemModule)
        {
            ...
            {
                GenerateSpecialNormal(rowName, Dictionary[columnName], mainModule, systemModule, method, specialIndex: 0);
            }
            else if (isColumnSpecial)
            {
                GenerateSpecialNormal(columnName, Dictionary[rowName], mainModule, systemModule, method, specialIndex: 1);
            }
            else
            {
        ...
        private void GenerateSpecialSpecial(string rowName, string columnName, ModuleDefinition mainModule, ModuleDefinition systemModule, MethodDefinition method)
        {
            ...
            DefineParameters(systemModule, Element0, Element1, key, method, WhereIndexEnumerable, T);
        ...
        private static void DefineParameters(ModuleDefinition systemModule, TypeReference TOuterElement, TypeReference TInnerElement, TypeReference TKey, MethodDefinition method, TypeReference WhereIndexEnumerable, TypeReference T)
        {
            var mainModule = method.Module;
            var outerSelector = new ParameterDefinition("outerSelector", ParameterAttributes.None, new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`2")))
            {
                GenericArguments = { TOuterElement, TKey }
            });
            method.Parameters.Add(outerSelector);

            var innerSelector = new ParameterDefinition("innerSelector", ParameterAttributes.None, new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`2")))
            {
                GenericArguments = { TInnerElement, TKey }
            });
            method.Parameters.Add(innerSelector);

            var comparer = new ParameterDefinition("comparer", ParameterAttributes.None, new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`3")))
            {
                GenericArguments = { TKey, TKey, mainModule.TypeSystem.Boolean }
            });
            method.Parameters.Add(comparer);

            var selector = new ParameterDefinition("selector", ParameterAttributes.None, new GenericInstanceType(mainModule.ImportReference(systemModule.GetType("System", "Func`3")))
            {
        ...
        {
            var TSelector = new GenericInstanceType(method.Module.GetType("UniNativeLinq", "DelegateFuncToStructOperatorFunc`3"))
            {
                GenericArguments =
                {
                    Element0,
        ...
        {
            return new GenericInstanceType(method.Module.GetType("UniNativeLinq", "DelegateFuncToStructOperatorFunc`2"))
            {
                GenericArguments = { Element0, TKey }
            };
        }

        private static TypeReference InternalInnerRoutine(TypeReference Element1, TypeReference TKey, ModuleDefinition MainModule, TypeReference TKeyEqualityComparer, out GenericInstanceType WhereIndexEnumerable)
        {
            var TInnerKeySelector = new GenericInstanceType(MainModule.GetType("UniNativeLinq", "DelegateFuncToStructOperatorFunc`2"))
            {
